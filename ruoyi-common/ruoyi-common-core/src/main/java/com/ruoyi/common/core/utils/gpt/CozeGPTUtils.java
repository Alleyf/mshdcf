...
import com.ruoyi.common.core.domain.CozeGPTBo;
...
//            获取请求信息
            ...
            if (response.isSuccessful()) {
                ...
                log.error("Coze请求失败：" + response.body().string());
//                log.error("Coze请求失败：" + JSON.parseObject(response.body().string()).get("message"));
//                throw new ServiceException(response.message());
            }

        } catch (Exception e) {
            log.error("Coze请求异常：" + e.getMessage());
            e.printStackTrace();
//            throw new ServiceException(e.getMessage());
    ...
    private static Map<String, String> getJsonRequest(String text) {
        return new HashMap<String, String>(2) {
            {
                put("url", GPTConstants.COZE_API_URL);
                put("text", cozeRequest(text));
            }
        };
    }

    private static String cozeRequest(String text) {
        CozeGPTBo cozeGPTBo = new CozeGPTBo();
        cozeGPTBo.setMessages("", text);
        return JSON.toJSONString(cozeGPTBo);
    }
...
//                        必须转化为json字符串才可以
                ...
                log.error("Coze请求失败: " + response.code() + "：" + response.message());
            }
        } catch (Exception e) {
            log.error("Coze请求异常: " + e.getMessage());
            e.printStackTrace();
    ...
    private static String cozeParseResponse(Response response) throws IOException {
